...
class Registration02Controller extends Controller
{
    /**
     * @Route("TeacherC02/register/{clientid}", name="registration_TCuser02")
     */
    public function registercli02Action(Request $request, UserPasswordEncoderInterface $passwordEncoder, $clientid)
    {
        // 1) build the form
        ...
        $Adresses = $em->getRepository(Adresses02::class)->findOneBy(array('clientid'=>$idClient));
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            return $this->redirectToRoute('Search_AllUser02');
        }

        return $this->render(
            'registration/register02.html.twig',
            array('form' => $form->createView())
        );
    }

    /**
     * @Route("TeacherE02/register/{employeeid}", name="registration_TEuser02")
     */
    public function Registeremp02Action(Request $request, UserPasswordEncoderInterface $passwordEncoder, $employeeid)
    {
        // 1) build the form
        $user = new UserControl();
        $clientd = new clients02();
        $emplo = new Employees02();
        $Adresses = new Adresses02();
        $em = $this->getDoctrine()->getManager();
       
        $emplo = $em->getRepository(Employees02::class)->find($idemployee);
        $adres = $em->getRepository(Adresses02::class)->findOneBy(array('employeeid'=> $employeeid));  
        $form = $this->createForm(UserType::class, $user);
	...
	/**
     * @Route("Clientregister02/{clientid}", name="registration_client02")
     */
    public function Clientregister02Action(Request $request, UserPasswordEncoderInterface $passwordEncoder, $clientid)
    {
        // 1) build the form
        $user = new UserControl();
        $form = $this->createForm(UserType::class, $user);
		...
		/**
     * @Route("Employee02/register/{employeeid}", name="registration_employee02")
     */
    public function Employeegister02Action(Request $request, UserPasswordEncoderInterface $passwordEncoder, $employeeid)
    {
        // 1) build the form
        $user = new UserControl();
        ...
        return $this->render(
            'registration/registeremployee02.html.twig',
            array('form' => $form->createView())
        );
    }
}